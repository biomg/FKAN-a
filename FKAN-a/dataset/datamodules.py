...
def drug_target_collate_fn(args: T.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]):
    ...
    # 取出边索引
    # drug_edge_indices = [a[2] for a in args]
    # target_edge_indices = [a[3] for a in args]
    ...
    labels = torch.stack(labs, 0)
     ...
     # Stack edge indices (assuming they are of the same length)
    # drug_edges = pad_sequence(drug_edge_indices, batch_first=True, padding_value=0)  # 根据实际情况选择填充值
    # target_edges = pad_sequence(target_edge_indices, batch_first=True, padding_value=0)  # 根据实际情况选择填充值
...
class BinaryDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        label = torch.tensor(self.labels.iloc[i])
         ...
         # 加载边索引
        # drug_edge_index = self.drugs.iloc[i]['Drug Edge Index']
        # target_edge_index = self.targets.iloc[i]['Target Edge Index']
