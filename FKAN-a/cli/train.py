...
def add_args(parser: ArgumentParser):
    ...
    # model_group.add_argument(
    #     "--hidden-size", help="hidden_size", dest="hidden_size"
    # )
    model_group.add_argument(
...
def step(model, batch, device=None):
    ...
    drug, target, label = batch  # 获取边索引
    pred = model(drug.to(device), target.to(device)).view(-1)  # 传递边索引
    label = Variable(torch.from_numpy(np.array(label)).float()).to(device).view(-1)
    return pred, label
...
def contrastive_step(model, batch, device=None):
    ...
    anchor, positive, negative = batch  # 获取边索引
    ...
    # # 使用边索引处理
    # anchor_projection = model.gnn(anchor_projection, anchor_edge_index)
    # positive_projection = model.gnn(positive_projection, positive_edge_index)
    # negative_projection = model.gnn(negative_projection, negative_edge_index)
...
def main(args):
    ...
    for epo in range(config.epochs):
        ...
        for i, batch in tqdm(
            ...
            loss = loss_fct(pred.view(-1), label.view(-1))
        ...
        # Contrastive Step
        if config.contrastive:
            logg.info(f"Training FKAN-a at Epoch {epo + 1}")
            for i, batch in tqdm(
