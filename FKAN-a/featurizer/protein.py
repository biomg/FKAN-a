...
class ESM2Featurizer(Featurizer):
    def __init__(self, save_dir: Path = Path().absolute()):
        ...
        (
            self._esm_model,
            self._esm_alphabet,
        ) = esm.pretrained.esm2_t33_650M_UR50D() # esm2_t33_650M_UR50D()为预训练蛋白质模型
...
class ESM1vFeaturizer(Featurizer):
    def __init__(self, save_dir: Path = Path().absolute()):
        super().__init__("ESM", 1280, save_dir)
...
class ESM2vFeaturizer(Featurizer):
    def __init__(self, save_dir: Path = Path().absolute()):
        super().__init__("ESM", 2560, save_dir)
